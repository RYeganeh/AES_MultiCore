...
private:
    ...
    void key_expansion(const Key256& key) {
...
public:
    ...
    static std::string key_to_hex(const Key256& key) {
    ...
    AES256(const Key256& key) {
        key_expansion(key);
        round_outputs_.reserve(70); // Pre-allocate for round outputs
    }
    ...
    Block encrypt_with_output(const Block& plaintext) {
        ...
        // Final round (14)
        sub_bytes(state);
        record_state("round[14].s_box", state);
        shift_rows(state);
        record_state("round[14].s_row", state);
        record_state("round[14].k_sch", state_to_bytes(round_keys_[14]));
        add_round_key(state, round_keys_[14]);
        record_state("round[14].output", state);
...
void test_aes() {
    ...
    std::cout << "PLAINTEXT: " << AES256::bytes_to_hex(plaintext) << "\n\n";
    std::cout << "KEY: " << AES256::key_to_hex(key) << "\n\n";
    ...
    if (ciphertext == expected_ciphertext) {
        std::cout << "\nCiphertext matches expected: " << AES256::bytes_to_hex(ciphertext) << '\n';
    } else {
        std::cout << "\nEncryption failed: " << AES256::bytes_to_hex(ciphertext)
                  << " != " << AES256::bytes_to_hex(expected_ciphertext) << '\n';
    }
}
