...
def cipher_with_output(plaintext, key):
    ...
    # Round 0
    print(f"round[ 0].input  {plaintext.hex()}")
    print(f"round[ 0].k_sch  {key.hex()}")
    state = add_round_key(state, round_keys[0])
    print(f"round[ 1].start  {state_to_hex(state)}")
    ...
    for round in range(1, 10):
        ...
        print(f"round[{round:2d}].k_sch  {state_to_bytes(round_keys[round]).hex()}")
        state = add_round_key(state, round_keys[round])
        print(f"round[{round + 1:2d}].start  {state_to_hex(state)}")
    ...
    print(f"round[10].k_sch  {state_to_bytes(round_keys[10]).hex()}")
...
def test_aes():
    ...
    print(f"KEY: {key.hex()}\n")
    ciphertext = cipher_with_output(plaintext, key)

    assert ciphertext == expected_ciphertext, f"Encryption failed: {ciphertext.hex()} != {expected_ciphertext.hex()}"
    print(f"\nCiphertext matches expected: {ciphertext.hex()}")
