...
std::string key_to_hex(const Key192& key) {
...
RoundKeys key_expansion(const Key192& key) {
...
Block cipher_with_output(const Block& plaintext, const Key192& key) {
    ...
    // Final round (12)
    sub_bytes(state);
    std::cout << "round[12].s_box  " << bytes_to_hex(state_to_bytes(state)) << '\n';
    shift_rows(state);
    std::cout << "round[12].s_row  " << bytes_to_hex(state_to_bytes(state)) << '\n';
    std::cout << "round[12].k_sch  " << bytes_to_hex(state_to_bytes(round_keys[12])) << '\n';
    add_round_key(state, round_keys[12]);
    std::cout << "round[12].output " << bytes_to_hex(state_to_bytes(state)) << '\n';
