...
void bytes_to_state(const Key& block, State& state) {
...
Key state_to_bytes(const State& state) {
...
std::string bytes_to_hex(const Key& bytes) {
...
Key cipher_with_output(const Key& plaintext, const Key& key) {
    ...
    std::cout << "round[ 0].k_sch  " << bytes_to_hex(key) << '\n';
    ...
    // Final round (10)
    sub_bytes(state);
    std::cout << "round[10].s_box  " << bytes_to_hex(state_to_bytes(state)) << '\n';
    shift_rows(state);
    std::cout << "round[10].s_row  " << bytes_to_hex(state_to_bytes(state)) << '\n';
    std::cout << "round[10].k_sch  " << bytes_to_hex(state_to_bytes(round_keys[10])) << '\n';
    add_round_key(state, round_keys[10]);
    std::cout << "round[10].output " << bytes_to_hex(state_to_bytes(state)) << '\n';

    return state_to_bytes(state);
}

void test_aes() {
    // Test with provided example from FIPS 197 Appendix C.1
    Key key = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
    Key plaintext = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff};
    Key expected_ciphertext = {0x69, 0xc4, 0xe0, 0xd8, 0x6a, 0x7b, 0x04, 0x30, 0xd8, 0xcd, 0xb7, 0x80, 0x70, 0xb4, 0xc5, 0x5a};

    std::cout << "PLAINTEXT: " << bytes_to_hex(plaintext) << "\n\n";
    std::cout << "KEY: " << bytes_to_hex(key) << "\n\n";
    Key ciphertext = cipher_with_output(plaintext, key);
