...
#define THREADS_PER_BLOCK 128 // Reduced to fit shared memory constraints
...
__global__ void aes_encrypt_kernel(const uint8_t* plaintexts, uint8_t* ciphertexts, int num_blocks) {
    ...
    __shared__ uint8_t shared_s_box[256];
    __shared__ uint8_t shared_round_keys[176];
    __shared__ uint8_t shared_state[THREADS_PER_BLOCK][4][4];
