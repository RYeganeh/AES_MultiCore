...
void test_file_encryption(void) {
    ...
    err = cudaMemcpy(d_plaintexts, plaintexts, plaintext_count * BLOCK_SIZE * sizeof(uint8_t), cudaMemcpyHostToDevice);
    if (err != cudaSuccess) {
        fprintf(stderr, "CUDA Error: %s\n", cudaGetErrorString(err));
        cudaFree(d_plaintexts);
    ...
    for (int iter = 0; iter < num_iterations; ++iter) {
        ...
        int blocks = (plaintext_count + threads_per_block - 1) / threads_per_block;
        aes_encrypt_kernel<<<blocks, threads_per_block>>>(d_plaintexts, d_ciphertexts, plaintext_count);
    ...
    err = cudaMemcpy(ciphertexts, d_ciphertexts, plaintext_count * BLOCK_SIZE * sizeof(uint8_t), cudaMemcpyDeviceToHost);
    if (err != cudaSuccess) {
        fprintf(stderr, "CUDA Error: %s\n", cudaGetErrorString(err));
        goto cleanup;
    }
