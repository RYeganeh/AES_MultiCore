...
void test_file_encryption() {
    ...
    if (!read_keys("plaintext_100_blocks.txt", plaintexts)) {
        std::cerr << "Failed to read plaintext_n_blocks.txt\n";
        return;
    }
    if (!read_keys("ciphertexts_100_blocks.txt", expected_ciphertexts)) {
    ...
    for (int i = 0; i < num_iterations; ++i) {
        std::cout << "Iteration " << (i + 1) << ": Encryption loop time: "
                  << std::fixed << std::setprecision(3) << times[i] << " ms\n";
    }

    std::cout << std::string(80, '=') << "\n";
    std::cout << "Average encryption loop time over " << num_iterations
              << " iterations: " << std::fixed << std::setprecision(3)
              << (total_time / num_iterations) << " ms\n" ;
    std::cout << std::string(80, '=') << "\n\n";
...
int main() {
    ...
    #pragma omp parallel
    {
        if (omp_get_thread_num() == 0) {  // Only the master thread prints
